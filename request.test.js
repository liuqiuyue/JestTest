...
// 最后一个测试用例，假设我们现在有一个返回的是 404 的接口，我们需要对这个接口测试，期望他返回 404。
//我们用 catch 捕获，在 catch 中判断。但是，假如这个接口返回的不是 404，而是正常返回 200，这个 catch 则不会执行，expect 也不会执行，测试依然是通过的。
//这不符合我们的预期！所以，我们需要加上 expect.assertions(1) 进行断言：下面一定会执行一个 expect